...
<!-- content1 -->
  ...
  <div class="col-lg-6">
    ...
    @foreach($dt_blog_portal_sma as $dt_blog_portal_sma)
    <a href="{{ url('view/'.$dt_blog_portal_sma->id) }}">
    <div class="content1-box">
      <img class="cb-img" src="{{ url('images/'.$dt_blog_portal_sma->cover_photo) }}" />
      <div class="cb-title">
        {!! $dt_blog_portal_sma->dt_blog_title !!}
      </div>
      <div class="cb-desc">
      <!-- {!! substr(preg_replace("/<img\s[^>]*?src\s*=\s*['\"]([^'\"]*?)['\"][^>]*?>/",'',$dt_blog_portal_sma->dt_blog_text),0,400) !!}...... -->
      <?php
            $string = strip_tags($dt_blog_portal_sma->dt_blog_text);
      ...
      <div class="cb-inf">
        <i class="fa fa-user"></i> {{$dt_blog_portal_sma->dt_blog_create_by}} ({{$dt_blog_portal_sma->dt_blog_by}})
        <p class="cb-date">
          {{$dt_blog_portal_sma->created_at}}
        </p>
      </div>
    </div>
    @endforeach
